...
import { API_URL, stages, templates } from '../constants';
import ResultsView from '../components/ResultsView';
import { Project, Template, BusinessAnalysis, AppPrototypeData } from '../types/project';
...
export default function MainGenerator({ onProjectComplete, currentProject }: MainGeneratorProps) {
    ...
    const handleGenerate = async () => {
        ...
        try {
            ...
            const appData: AppPrototypeData = await appResponse.json();
            const analysisData: { analysis: BusinessAnalysis } = await analysisResponse.json();
            ...
            const project: Project = {
                ...
                targetAudience,
                budget,
                app: appData.code ? appData : { code: '<h1>Error loading prototype</h1>', url: undefined }, // Type check
            ...
            clearInterval(interval);
            const errorMessage = err instanceof Error ? err.message : 'Failed to generate startup package';
    ...
    if (currentProject) {
        return <ResultsView project={currentProject} onStartNew={() => onProjectComplete(null as unknown as Project)} />;
    }
